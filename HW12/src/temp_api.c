...
int8_t get_month_average_temp(temperature_record_t* data, uint16_t size,
...
int8_t get_month_min_temp(temperature_record_t* data, uint16_t size,
                          uint8_t month, int8_t* p_min_temp) {
    int8_t result = 0;  // true if valid data exists in month
    // Find first
    for (uint16_t i = 0; i < size; i++) {
        // Skip not valid data or other month
        if (!data[i].valid || data[i].month != month) {
            continue;
        }
        int8_t current_temp = data[i].temperature;
        // First founded temp;
        if (!result) {
...
int8_t get_month_max_temp(temperature_record_t* data, uint16_t size,
...
int8_t get_year_average_temp(temperature_record_t* data, uint16_t size,
...
int8_t get_year_min_temp(temperature_record_t* data, uint16_t size,
                         int8_t* p_min_temp) {
    int8_t result = 0;  // true if valid data exists in year
    for (size_t i = 0; i < size; i++) {
        // Skip not valid data
...
int8_t get_year_max_temp(temperature_record_t* data, uint16_t size,
                         int8_t* p_max_temp) {
    int8_t result = 0;  // true if valid data exists in year
    for (size_t i = 0; i < size; i++) {
        // Skip not valid data
...
static void sort_temperature_records_by_date(temperature_record_t* data,
                                             uint16_t count) {
    qsort(data, count, sizeof(temperature_record_t),
...
void print_temperature_records(temperature_record_t* data, uint16_t count) {
    ...
    for (uint16_t i = 0; i < count; i++) {
        ...
        print_record(&data[i], increment_position++);
    }
}

void print_temperature_records_by_month(temperature_record_t* data,
                                        uint16_t count, uint8_t month) {
    sort_temperature_records_by_date(data, count);
    uint16_t increment_position = 1;
    for (uint16_t i = 0; i < count; i++) {
        if (!data[i].valid || data[i].month != month) {
