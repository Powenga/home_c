...
static uint8_t days_in_month(uint16_t year, uint8_t month) {
  ...
  if (month == 2)
    return days[1] + is_leap_year(year);
...
void print_help(char *prog) {
  ...
  printf("\t-h - Help text;  \n");
  printf("\t-f path/to/file.csv - Specify file with temperature "
         "data;"
         "\n");
  printf("\t-m month_number - Specify month number to show month statistics"
         "\n");
}
