// Все нечетные
// Дана последовательность целых чисел через пробел, завершающаяся числом 0.
// Выведите все нечетные числа из этой последовательности, сохраняя их порядок.

// Формат входных данных
// Последовательность не нулевых целых чисел. В конце число 0.
// Формат результата
// Введенная последовательность, кроме четных чисел в том же порядке. Число 0 не
// выводить. Примеры Входные данные 12 13 173 28 19 0 Результат работы 13 173 19

#include <stdio.h>

int main(void) {
    int input, count = 0;
    while (scanf("%d", &input)) {
        if (input == 0) {
            break;
        }
        if (input % 2 != 0) {
            printf("%d ", input);
        }
    }
    return 0;
}
